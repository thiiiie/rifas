...
export default function KitPoloPage() {
  return (
    <div className="flex flex-col items-center">
      ...
      <main className="relative -top-2 max-w-[600px] w-full bg-rifa-background p-4 rounded-2xl">
        <section className="bg-background rounded-2xl">
          <Image className="rounded-2xl w-full aspect-[5/4] object-cover rounded-b-none" width={600} height={600} priority src={products.kitPolo.image} alt="imagem do produto" />

          <div className="p-4">
            <h1 className="text-md font-medium leading-5">{products.kitPolo.name}</h1>
            <p className="text-xs text-gray-600">{products.kitPolo.description}</p>
        ...
        <p className="text-center flex items-center justify-center gap-2 text-sm mt-2">
          ...
          <span className="p-2 bg-selectButton-background text-selectButton-foreground rounded-lg font-bold">
            {
              products.kitPolo.price.toLocaleString('pt-BR', {
                style: 'currency',
                currency: 'BRL'
              })
            }
        ...
        <RifaForm price={products.kitPolo.price} productId={products.kitPolo.id} />
